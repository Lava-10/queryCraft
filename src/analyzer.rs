...
use crate::{
    db::{DatabaseContext, DbError, Schema, SqlError, TableMetadata, practDb_META, ROW_ID_COL},
...
pub(crate) enum AnalyzerError {
    ...
    RowIdAssignment,
    /// Attempt to modify the internal [`practDb_META`] table.
    practDbMetaModification,
}
...
impl Display for AnalyzerError {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            ...
            Self::practDbMetaModification => write!(
                f,
                "table '{practDb_META}' is reserved for internal use, it cannot be manually changed or created"
            ),
        }
    }
}
...
pub(crate) fn analyze(
    ...
    match statement {
        ...
        Statement::Insert {
            ...
            if into == practDb_META {
                return Err(AnalyzerError::practDbMetaModification.into());
            }
